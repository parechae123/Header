...
{
    private Dictionary<Vector2Int,Interactions> interactionGrid = new Dictionary<Vector2Int,Interactions>();
    ...
    public bool InteractionInitOutIt(Vector3 plrPos,ref Vector2Int lastPos)
    {
        ...
        if (isInteractionAreThere.Contains(tempBigPos))
        {
            if (isInteractionAreThere.Contains(lastPos))
            {
                interactionGrid[lastPos].OutIt();
            }
                interactionGrid[tempBigPos].Init();
            lastPos = tempBigPos;
            return true;
        }
        else
